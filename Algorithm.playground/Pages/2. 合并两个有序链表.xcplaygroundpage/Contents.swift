...
/*
 ...
 headNode =  ListNode.init(node1!.val);
 headNode?.next = node1?.next
 }else {
 headNode = ListNode.init(node2!.val);
...
//输入链表1和2
var node1: ListNode? = creatList1()
var node2: ListNode? = creatList2()

//输出新链表
var newHeadNode: ListNode? = mergeTwoLists(&node1, &node2)
